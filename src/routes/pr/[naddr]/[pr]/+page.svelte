	...
	$effect(() => {
		...
		ndk.fetchEvent(pr).then((event) => {
			...
			if (eTagFork) {
				...
				ndk
					.fetchEvents({ ids: [eTagFork] }, { cacheUsage: NDKSubscriptionCacheUsage.ONLY_RELAY })
					.then((events) => {
...
{#if originalEvent && prEvent && proposedVersion}
	<h3 class="flex flex-row items-center gap-1 justify-center">
		<Avatar {ndk} pubkey={prEvent.pubkey} class="w-10 h-10 rounded-full object-cover" />
		<Name {ndk} pubkey={prEvent.pubkey} />
	...
	{#if diff}
		<Button onclick={() => (showAll = !showAll)}>Show All</Button>
		<div class="grid grid-cols-2 gap-4">
			{#each diff as part}
				<div
					class="p-4 bg-black/10 flex flex-row"
					class:col-span-2={!part.added && !part.removed}
					class:hidden={!part.added && !part.removed && !showAll}
				>
					{#if part.added}
	...
	<div class="flex flex-row gap-4 my-4">
		<Card.Root class="w-1/2">
			<Card.Header>
				<Card.Title>Your Version</Card.Title>
			</Card.Header>
			...
			<Card.Content>
				...
				{:else}
					<div class="grid w-full items-center gap-4">
						<Editor bind:content bind:title />

						<Button class="w-fit px-10" onclick={save}>Save</Button>
					</div>
				{/if}
			</Card.Content>
		</Card.Root>
		...
		<Card.Root class="w-1/2">
			<Card.Header>
				<Card.Title>
					<Name {ndk} pubkey={proposedVersion.pubkey} />'s Version
				</Card.Title>
			</Card.Header>
